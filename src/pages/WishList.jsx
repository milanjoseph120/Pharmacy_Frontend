  ...
  <div>
     <Header/>
      <Row className='ms-md-5 me-md-5 '>
        {wishlistArray?.length > 0 ? (
          wishlistArray.map((itemss) => (
            <Col key={itemss.id} style={{marginTop:"100px"}} xs={12} sm={6} md={4} lg={3}>
              <Card style={{ height: '100%' }} className='btn shadow'>
                <Card.Img
                  variant="top"
                ...
                <Card.Body>
                  <Card.Title>{itemss.name}</Card.Title>
                  {/* Uncomment the following lines if you want to display the price */}
                  {/* <Card.Text>
                    <p>Price: {itemss.price}</p>
                  </Card.Text> */}
                  {/* <div className='d-flex align-items-center justify-content-between'>
